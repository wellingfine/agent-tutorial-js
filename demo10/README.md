...
- 切分发生在 `document_loader.js` 的 `splitText()`，向量化发生在 `embeddings.js` 的 `embedTexts()`：文本进去，768 维浮点数组出来
- chunk 总数要远大于 `TOP_K`，否则 Top-K 会返回全部片段，检索这一步等于没做
...
- 把 `TOP_K` 改成 20（大于当前 chunk 总数），观察检索是否退化成“把整篇知识库都塞进 Prompt”
